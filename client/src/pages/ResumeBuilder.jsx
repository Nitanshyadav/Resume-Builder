...
import PersonalInfoForm from "../Components/PerformanceInfoForm";
...
const ResumeBuilder = () => {
  ...
  return ( 
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      <Link to="/app" className='inline-flex gap-2 items-center hover:text-slate-700 transition-all'>
            ...
            <div className='flex justify-between items-center mb-6 border-b border-gray-300 py-1'>
              ...
              <div className='flex items-center'>
                {activeSectionIndex !== 0 && (
                  <button onClick={() =>setActiveSection((prevIndex)=>Math.max(prevIndex-1,0))} className='flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all' disabled={activeSectionIndex===0}>
                    <ChevronLeft className='size-4'/>Previous
                  </button>
                )}
                 <button onClick={() => setActiveSection((prevIndex)=>Math.min(prevIndex+1,sections.length-1))} className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === sections.length - 1 && 'opacity-50'}`} disabled={activeSectionIndex===sections.length-1}>
            ...
            <div className="lg:col-span-7 max-lg:mt-6">
                <div className='relative w-full'>
                  <div className='absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2'>
                    ...
                    <button onClick={downloadResume} className='flex items-center gap-2 px-6 py-2 text-xs bg-gradient-to-br from-gray-100 to-green-200 text-green-600 rounded-lg ring-green-300hover:ring transition-colors'>
