...
const ColorPicker = ({ SelectedColor,onChange }) =>{
  ...
  return (
    <div className='relative'>
      <button onClick={()=>setIsOpen(!isOpen)} className='flex items-center gap-1 text-sm text-purple-600 bg-gradint-to-br from-purple-50 to-purple-100 ring-purple-300 hover:ring trnasition-all px-3 py-2 rounded-lg'>
      ...
      {isOpen && (
        <div className='grid grid-cols-4 w-60 gap-2 absolute top-full left-0 right-0 p-3 mt-2 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
          {colors.map((color)=>(
            <div key={color.value} className='relative cursor-;ointer group flex flex-col' onClick={()=>{onChange(color.value)}}>
